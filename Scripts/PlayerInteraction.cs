...
public class PlayerInteraction : MonoBehaviour
{
    ...
    /*
        void TryInteract()
        {
            ...
            if (carriedItem != null)
            {
                InteractibleItem carriedInteracible = carriedItem.GetComponent<InteractibleItem>();
                if (Physics.Raycast(Camera.main.transform.position, Camera.main.transform.forward, out hit, interactRange))
                {
                    AttachmentPoint attachmentPoint = hit.collider.GetComponent<AttachmentPoint>();
                    if (attachmentPoint != null && carriedInteracible != null && carriedInteracible.attachmentType == attachmentPoint.acceptedType)
                    {
                        AttachItemToPoint(carriedInteracible, attachmentPoint);
                        return;
                    }
                }
                if (Physics.Raycast(Camera.main.transform.position, Camera.main.transform.forward, out hit, interactRange))
            ...
            {
                raycastMask &= ~(1 << attachmentPointLayer);
            }
            if (Physics.Raycast(Camera.main.transform.position, Camera.main.transform.forward, out hit, interactRange))
            {
                ...
                if (item != null)
                {
                    ...
                    if (item.currentState == AttachmentState.Fixed)
                    {
                        ...
                        if (seat != null && !seat.isOccupied)
                        {
                            ...
                            seat.Sit(gameObject);
                            currentSeat = seat;
                            //Disable player movement controls - TODO: enable car controls
                        ...
                        if (IsPartOfAssembledItem(item))
                        {
                            ...
                            // ✅ DETACH LOOSE PARTS before handling the pickup
