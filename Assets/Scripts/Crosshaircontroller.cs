...
public class CrosshairController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            ConsumableItem heldConsumable = player.GetCarriedItem().GetComponent<ConsumableItem>();
            if (heldConsumable != null)
            {
                SetCrosshairAlpha(interactAlpha);
                ShowTooltip($"Press F to consume {heldConsumable.displayName}");
                return; // Don't process any other crosshair logic while holding an item
            }
        }
