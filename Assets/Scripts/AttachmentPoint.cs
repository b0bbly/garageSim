...
public class AttachmentPoint : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (isOccupied)
        {
            // Check if any colliders got re-enabled
            Collider[] allColliders = GetComponents<Collider>();
            foreach (var col in allColliders)
            {
                if (col.enabled)
                {
                    Debug.LogWarning($"Collider {col.name} on {gameObject.name} was re-enabled while occupied!");
                    col.enabled = false;
                }
            }
            MeshCollider[] allMeshcolliders = GetComponents<MeshCollider>();
            {
                foreach (var col in allMeshcolliders)
                {
                    if (col.enabled)
                    {
                        Debug.LogWarning($"Collider {col.name} on {gameObject.name} was re-enabled while occupied!");
